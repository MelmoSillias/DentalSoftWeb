$(function () {
  ...
  $("#confirmCancelBtn").on("click", function () {
    ...
    $.ajax({
      url: `/api/consultations/${selectedConsultationId}`,
      type: "DELETE",
      success: function () {
  ...
  $datepickerInput
    ...
    .on("changeDate", function (e) {
      ...
      if ($consultationTable.DataTable().ajax.url()) {
        $consultationTable.DataTable().ajax.reload();
      }
    });
  ...
  $(document).on("click", ".btn-view-consultation", function () {
    ...
    $.getJSON(`/admin/consultation/${id}/details.json`, function (data) {
      ...
      const modal = new bootstrap.Modal(
        document.getElementById("consultationDetailsModal"),
      );
      modal.show();
    }).fail(function () {
      showToastModal({
