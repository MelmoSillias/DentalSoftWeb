$(document).ready(function () {
    ...
    const employeeTable = $('#employeeTable').DataTable({
        ajax: {
            ...
            error: function (xhr, error, thrown) {
                ...
                const errorMessage = resolveApiErrorMessage(xhr, 'Erreur lors du chargement des données. Voir la console.');
                showToastModal({
        ...
        rowGroup: {
            dataSrc: function (row) {
                return (row.type || 'Non défini').trim();
            }
        },
        language: {
    ...
    $('#employeeForm').on('submit', function (e) {
        ...
        $.ajax({
            ...
            error: function (xhr) {
                const errorMessage = resolveApiErrorMessage(xhr, "Une erreur est survenue lors de la création de l'employé.");
                showToastModal({
                  message: errorMessage,
                  type: 'error',
                  duration: 3000
    ...
    function resolveApiErrorMessage(xhr, fallbackMessage) {
