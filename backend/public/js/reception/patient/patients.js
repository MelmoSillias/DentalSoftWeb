$(document).ready(function () {
    ...
    $('#consultationForm').submit(function (e) {
        ...
        const consultationTime = $('#consultationTime').val();

        if (!consultationDate || !consultationTime) {
            showToastModal({
                message: 'Veuillez renseigner la date et l\'heure de la consultation',
                type: 'error',
                duration: 3000
            });
            return;
        ...
        $.ajax({
            url: `/api/patient/${formData.patient_id}/consultation/create`,
            type: 'POST',
    ...
    $('#createConsultationModal').on('show.bs.modal', function () {
        ...
        $('#consultationDate').val(yyyyMmDd);
        $('#consultationTime').val(hhMm);
    });
