...
<template>
    <div ref="containerRef" class="w-full select-none rounded-xl border border-gray-200 bg-white p-2 xs:p-3 shadow-sm
           dark:border-gray-700 dark:bg-gray-900">
        <div class="flex items-center justify-between gap-1 xs:gap-2"> 
            <button @click="prevDay" class="flex h-8 xs:h-9 w-8 xs:w-9 items-center justify-center rounded-lg
            ...
            <div class="relative flex flex-1 justify-center overflow-hidden">
                <transition-group name="date-slide" tag="div" class="flex gap-1 xs:gap-2">
                    <button v-for="item in dates" :key="item.key" @click="selectDate(item.date)" class="flex h-9 xs:h-10 w-[72px] xs:w-[88px] flex-col items-center justify-center rounded-lg text-xs xs:text-sm
                   transition-all duration-300
                   dark:text-gray-300" :class="isSelected(item.date)
            ...
            <button @click="nextDay" class="flex h-8 xs:h-9 w-8 xs:w-9 items-center justify-center rounded-lg
               text-gray-600 hover:bg-gray-100
               dark:text-gray-300 dark:hover:bg-gray-800">
                ›
            </button>
