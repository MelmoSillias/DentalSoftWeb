...
const medecinLabel = computed(() => {
    ...
    return props.rdv.medecinName || props.rdv.medecin || props.rdv.medecin?.fullname || props.rdv.medecin?.name || `${props.rdv.medecin?.prenom ?? ''} ${props.rdv.medecin?.nom ?? ''}`.trim() || props.medecinName || 'Médecin';
});
...
<template>
    <div class="group relative h-full min-h-[112px] xs:min-h-[128px] bg-surface-0 p-2 xs:p-3 shadow-sm transition-all duration-200 hover:shadow-md dark:bg-surface-900 dark:shadow-none dark:ring-1 dark:ring-surface-800/80">
        ...
        <div class="mb-1.5 xs:mb-2.5 flex items-center justify-between text-[10px] xs:text-xs font-medium text-surface-500 dark:text-surface-400">
            <span>{{ slotLabel }}</span>
            <Button v-if="!hasRdv" icon="pi pi-plus" rounded text severity="secondary" size="small" class="opacity-70 hover:opacity-100 transition-opacity" @click.stop="emit('create')" />
        </div>

        <!-- Contenu RDV -->
        <Card v-if="hasRdv" class="h-full border border-surface-200 bg-surface-50/80 shadow-sm transition-colors hover:border-primary-300 dark:border-surface-700 dark:bg-surface-800 dark:hover:border-primary-700/50">
            <template #content>
                <div class="flex flex-col h-full">
                    <div class="mb-1.5 xs:mb-2.5 flex items-start justify-between gap-2 xs:gap-3">
                        ...
                        <Tag :severity="getSeverity(statutValue)" :value="getLabel(statutValue)" class="text-[10px] xs:text-xs font-medium whitespace-nowrap" />
                    </div>
                    ...
                    <Tag v-if="smsReminder" :severity="smsSeverity" :value="smsLabel" class="mb-3 w-fit text-[10px] xs:text-xs font-medium whitespace-nowrap" />

                    <div v-if="statutValue === 0" class="mt-auto flex flex-wrap gap-1 xs:gap-2">
                        <Button size="small" icon="pi pi-check" label="Valider" class="text-[10px] xs:text-xs" @click="trigger('validate')" />
                        <Button size="small" icon="pi pi-calendar-minus" label="Reporter" severity="warning" text class="text-[10px] xs:text-xs" @click="trigger('report')" />
                        <Button size="small" icon="pi pi-times" label="Annuler" severity="danger" text class="text-[10px] xs:text-xs" @click="trigger('cancel')" />
                    </div>
                </div>
            </template>
        </Card>
