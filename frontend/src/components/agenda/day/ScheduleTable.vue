...
<template>
    ...
    >
        <!-- En-tête -->
        <div class="sticky top-0 z-10 grid border-b border-surface-200 bg-gradient-to-b from-surface-100 to-surface-50 dark:border-surface-700 dark:from-surface-800 dark:to-surface-900" :style="{ gridTemplateColumns }">
            ...
            <div v-for="med in medecins" :key="med.id" class="flex items-center justify-center gap-2.5 border-l border-surface-200/80 px-3 py-3 dark:border-surface-700/80">
                <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary-500/15 text-xs font-bold text-primary-700 ring-1 ring-primary-500/25 dark:bg-primary-400/15 dark:text-primary-300 dark:ring-primary-400/30">
                ...
                <div class="min-w-0 text-left">
                    ...
                    <p class="truncate text-[10px] font-medium uppercase tracking-wide text-surface-500 dark:text-surface-400">Médecin</p>
                </div>
            </div>

            <div v-if="!medecins.length" class="flex items-center justify-center border-l border-surface-200 px-3 py-3 text-xs text-surface-500 dark:border-surface-700 dark:text-surface-400">Aucun médecin</div>
        </div>

        <!-- Corps -->
        <div class="divide-y divide-surface-100 bg-surface-0 dark:divide-surface-800 dark:bg-surface-900">
            <div v-for="slot in slots" :key="slot.minutes" class="grid" :style="{ gridTemplateColumns }">
                <div class="flex items-center justify-center border-r border-surface-200 bg-surface-50 px-2 py-2 text-xs font-semibold tabular-nums text-surface-600 dark:border-surface-700 dark:bg-surface-950 dark:text-surface-300">
                    {{ slot.label }}
                </div>

                <template v-if="medecins.length">
                    <div v-for="med in medecins" :key="med.id" class="border-l border-surface-200 bg-surface-0 transition-colors hover:bg-primary-50/40 dark:border-surface-700 dark:bg-surface-900 dark:hover:bg-primary-950/25">
                        <ScheduleCell
                ...
                <div v-else class="border-l border-surface-200 bg-surface-0 dark:border-surface-700 dark:bg-surface-900" />
            </div>
        </div>
    </div>
</template>
