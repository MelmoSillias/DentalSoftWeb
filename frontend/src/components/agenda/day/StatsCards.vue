...
<template>
  <Card class="dark:bg-surface-900">
    ...
    <template #title>
      ...
      >
        <span class="text-base xs:text-lg font-semibold text-surface-800 dark:text-surface-50">
    ...
    <template #content>
      ...
      >
        ...
        <div
          v-if="expanded"
          key="expanded"
          class="grid grid-cols-1 gap-3 xs:gap-4 sm:grid-cols-2 lg:grid-cols-4"
        >
          <Card
            v-for="item in items"
            :key="item.key"
              ...
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <Skeleton
                    v-if="loading"
                    width="3rem xs:4rem"
                    height="2rem xs:2.5rem"
                    borderRadius="8px"
                  />
                  <p
                    v-else
                    class="text-3xl xs:text-4xl font-bold"
                    :class="item.color"
                  >
        ...
        <div
          v-else
          key="collapsed"
          class="grid grid-cols-2 gap-2 xs:gap-3 sm:grid-cols-4"
        >
          <div
            v-for="item in items"
            :key="item.key"
            class="flex items-center justify-between rounded-lg
                   bg-surface-100 p-2 xs:p-3 shadow-sm
                   dark:bg-surface-800"
          >
            <Skeleton
              v-if="loading"
              width="2rem xs:2.5rem"
              height="1.5rem xs:1.75rem"
              borderRadius="6px"
            />
            <span
              v-else
              class="text-xl xs:text-2xl font-semibold"
              :class="item.color"
            >
              {{ props.stats[item.key] ?? 0 }}
            </span>
