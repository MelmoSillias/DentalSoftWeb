...
const menuItems = computed(() => {
    const items = [
        {
            ...
            command: () => {
                if (selectedEvent.value) {
                    emit('request-validate', selectedEvent.value.extendedProps);
                }
            }
        },
        {
            ...
            command: () => {
                if (selectedEvent.value) {
                    emit('request-report', selectedEvent.value.extendedProps);
                }
            }
        },
        {
            ...
            command: () => {
                if (selectedEvent.value) {
                    emit('request-cancel', selectedEvent.value.extendedProps);
                }
            }
        }
    ];
    ...
    if (isInternetFeaturesEnabled.value) {
        items.push(
            {
                label: 'Envoyer rappel SMS',
                icon: 'pi pi-send',
                command: () => {
                    if (selectedEvent.value) {
                        emit('request-sms-reminder', selectedEvent.value.extendedProps);
                    }
                }
            },
            {
                label: 'Programmer rappel auto',
                icon: 'pi pi-clock',
                command: () => {
                    if (selectedEvent.value) {
                        emit('request-sms-schedule', selectedEvent.value.extendedProps);
                    }
                }
            }
        );
    ...
    <section class="weekly-view-page flex flex-col gap-3 xs:gap-4 p-0.5 xs:p-1">
        ...
        <div data-tour="agenda-rdv.scope" class="flex flex-row items-center gap-3 xs:gap-4 rounded-xl xs:rounded-2xl bg-white p-3 xs:p-4 shadow-sm ring-1 ring-gray-200/70 dark:bg-gray-800 dark:ring-gray-700/60 dark:shadow-gray-900/20">
            ...
            <span class="p-input-icon-left w-full xs:w-72 min-w-[160px] xs:min-w-[180px]">
                <InputText icon="pi pi-search" v-model.trim="filters.patient" placeholder="Rechercher patient..." class="w-full" />
            </span>
            <MultiSelect v-model="filters.statuses" :options="statusOptions" optionLabel="label" optionValue="value" placeholder="Statuts" display="chip" class="w-full xs:w-80 min-w-[180px]" />
        </div>
...
.weekly-view-page :deep(.app-dark .fc),
.weekly-view-page :deep([class*='app-dark'] .fc) {
