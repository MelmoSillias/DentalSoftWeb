        ...
        <Dialog :visible="payDialogVisible" :header="payDialogHeader" :modal="true" :style="{ width: '760px' }" @update:visible="emit('update:payDialogVisible', $event)">
            <div class="flex flex-col gap-5">
                <Tabs v-if="showPayTabs" :value="String(activePayTabId ?? '')" @update:value="emit('update:activePayTabId', $event)">
                    <TabList>
                        <Tab v-for="tab in payTabs" :key="tab.id" :value="String(tab.id)">
                            {{ tab.label }}
                        </Tab>
                    </TabList>
                </Tabs>
                ...
                <template v-if="isValidateMode">
                    <div class="rounded-2xl border border-blue-200 bg-blue-50/70 p-4 dark:border-blue-800 dark:bg-blue-950/20">
                        <p class="text-sm text-surface-700 dark:text-surface-200">Confirmer que cette facture est vide et doit être marquée comme validée.</p>
                        <p v-if="selectedFacture?.id" class="mt-2 text-xs text-surface-500">Facture #{{ selectedFacture.id }}</p>
                    </div>
                </template>
                ...
                <template v-else-if="isSettledMode">
                    <div class="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4 dark:border-emerald-800 dark:bg-emerald-950/20">
                        <p class="text-sm text-surface-700 dark:text-surface-200">Cette facture est réglée. Sélectionnez une autre facture pour régler le reliquat.</p>
                        <p v-if="selectedFacture?.id" class="mt-2 text-xs text-surface-500">Facture #{{ selectedFacture.id }}</p>
                    </div>
                </template>
            ...
            <template #footer>
                <div class="flex items-center justify-between w-full gap-2 pt-1 flex-wrap">
                    <div class="flex items-center gap-3 min-w-0">
                        <span v-if="Number(priorReliquatTotal) > 0" class="inline-flex items-center gap-1.5 rounded-lg bg-red-50 px-2.5 py-1.5 text-xs font-semibold text-red-700 dark:bg-red-950/40 dark:text-red-300">
                    ...
                    <div class="flex items-center gap-2 ml-auto">
                        ...
                        <Button v-if="isValidateMode" label="Valider" severity="success" icon="pi pi-check" @click="emit('confirm-validate')" :loading="validateLoading || payLoading" class="text-sm font-semibold" />
                        <Button v-else-if="!isSettledMode" label="Confirmer le paiement" severity="success" icon="pi pi-check" @click="emit('submit-payment')" :loading="payLoading" class="text-sm font-semibold" />
                    </div>
                </div>
            </template>
        </Dialog>
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="rounded-xl border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800/30 p-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <Button icon="pi pi-plus" label="Ajouter un soin" size="small" class="rounded-xl border-0 bg-gradient-to-r from-blue-500 to-blue-600 px-4 py-2.5 text-white shadow-sm transition-all hover:shadow-md" @click="addFactureLine" />
                    </div>
                </div>
                            ...
                            <TabPanel value="paiements">
                                <div class="flex flex-col gap-4">
                                    ...
                                    <div v-if="previewPayments.length" class="flex flex-col gap-3">
                                        <div
                                            v-for="payment in previewPayments"
                                            :key="`${payment.sourceType}-${payment.id}`"
                                            class="preview-payment-card rounded-2xl border border-surface-200 bg-white/90 dark:bg-surface-800/80 p-4 shadow-sm dark:border-surface-700"
                                        >
