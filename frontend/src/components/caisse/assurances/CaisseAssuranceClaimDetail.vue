...
const emit = defineEmits(['back', 'collect-patient-share', 'print-receipt', 'print-claim', 'print-claim-devis']);
    ...
    <div class="flex flex-col gap-6">
        ...
        <template v-else-if="claim">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                    <span class="muted-text">Date facture:</span> <span class="page-title">{{ claim.dateFacture?.slice(0, 16) || '—' }}</span>
                </div>
                <div>
                    <span class="muted-text">Téléphone:</span> <span class="page-title">{{ claim.telephone || '—' }}</span>
                </div>
                <div v-if="claim.lotId">
                    <span class="muted-text">Lot:</span> <span class="page-title">#{{ claim.lotId }} ({{ claim.lotStatut }})</span>
                </div>
            </div>

            <div class="flex flex-wrap gap-2">
                <Button v-if="claim.availableActions?.canCollectPatient" icon="pi pi-wallet" label="Payer" size="small" severity="success" :loading="actionLoading" @click="emit('collect-patient-share', claim)" />
            ...
            <div class="section-card p-4 rounded-xl">
                ...
                <DataTable v-else :value="claim.paiements" size="small">
                    ...
                    <Column header="Actions">
                        <template #body="{ data }">
                            <Button v-if="data.paiementId" icon="pi pi-print" label="Reçu" size="small" text @click="emit('print-receipt', data)" />
                        </template>
                    </Column>
                </DataTable>
            </div>
