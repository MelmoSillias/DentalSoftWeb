        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="muted-text text-sm">{{ lot?.assurance?.nom }} · {{ lot?.dateDebut || '—' }} → {{ lot?.dateFin || '—' }}</p>
                </div>
                <Tag v-if="lot" :value="lotStatutTag(lot.statut).label" :severity="lotStatutTag(lot.statut).severity" />
            </div>
            <div class="flex flex-wrap gap-2">
                ...
                <Button v-if="lot?.availableActions?.canUnconfirm" icon="pi pi-undo" label="Retour envoyé" outlined :loading="actionLoading" @click="emit('unconfirm-lot', lot)" />
                <Button v-if="lot?.availableActions?.canRefund" icon="pi pi-wallet" label="Rembourser" severity="success" :loading="actionLoading" @click="openRefundDialog" />
            </div>
        </div>
            ...
            <div class="section-panel">
                ...
                <DataTable :value="lot.remboursements || []" striped-rows size="small" empty-message="Aucun remboursement">
                    ...
                    <Column header="Actions">
                        <template #body="{ data }">
                            <Button v-if="data.canCancel" icon="pi pi-times" label="Annuler" size="small" severity="danger" outlined :disabled="actionLoading" @click="emit('cancel-refund', { lot, transaction: data })" />
                        </template>
                    </Column>
                </DataTable>
            </div>
        ...
        <Dialog v-model:visible="refundDialogVisible" modal header="Remboursement du lot" class="w-full max-w-md">
            <div class="flex flex-col gap-4 py-2">
                <div>
                    ...
                    <Select v-model="refundForm.modeId" :options="paymentMethods" option-label="libelle" option-value="id" class="w-full" placeholder="Choisir un mode" />
                </div>
                <div>
