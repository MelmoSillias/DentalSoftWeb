...
<template>
    <div class="flex flex-col gap-8">
        <div class="section-card p-5">
            ...
            <div v-else-if="!cards.length" class="py-12 text-center muted-text">Aucune assurance active.</div>

            <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                <article v-for="card in cards" :key="card.id || card.code" class="assurance-card group cursor-pointer" @click="emit('view-lots', card)">
                    <div class="assurance-card-brand">
                        <div class="assurance-logo">
                            <img v-if="resolveAssuranceLogoUrl(card.logoPath)" :src="resolveAssuranceLogoUrl(card.logoPath)" :alt="card.nom" class="assurance-logo-img" />
