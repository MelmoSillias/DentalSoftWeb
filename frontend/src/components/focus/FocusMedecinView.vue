...
<template>
    <div class="h-full min-h-0 bg-surface-50 dark:bg-surface-950 shadow-sm overflow-hidden">
        <Splitter class="h-full border border-surface-200 dark:border-surface-700 rounded-2xl overflow-hidden shadow-sm" :min-size="[220, 400, 240]">
            <!-- ==================== PANNEAU GAUCHE : DOSSIER PATIENT ==================== -->
            <SplitterPanel :size="20" class="flex flex-col">
                <div class="p-2 bg-white dark:bg-surface-900 flex flex-col border-r border-surface-200 dark:border-surface-700 relative h-full overflow-y-auto">
                    <div class="absolute top-4 right-4 z-10">
                        <Button v-if="selectedPatient" icon="pi pi-times" severity="danger" text size="small" @click="emit('clear-selection')" />
                    </div>
                    ...
                    <div v-else-if="selectedPatient && hidePatientDossier" class="flex-1 flex flex-col items-center justify-center text-center text-surface-400 my-10">
                        <i class="pi pi-lock text-5xl mb-4 opacity-75"></i>
                        <p class="text-base">Dossier patient masqué</p>
                    </div>

                    <div v-else class="flex-1 flex flex-col items-center justify-center text-center text-surface-400 my-10">
            ...
            <SplitterPanel :size="56" :minSize="40" class="flex flex-col overflow-y-auto bg-surface-50 dark:bg-surface-950">
                <div v-if="canShowEmbeddedWorkspace" class="flex-1 border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 overflow-y-auto">
                ...
                <div v-else class="flex-1 flex flex-col items-center pt-5 text-surface-400">
                    ...
                    <p class="text-sm mt-2 max-w-xs text-center">Sélectionnez une consultation dans la file d'attente pour commencer</p>
                </div>
            </SplitterPanel>
            ...
            <SplitterPanel :size="20" class="flex flex-col">
                <div class="h-full flex flex-col bg-white dark:bg-surface-900 border-l border-surface-200 dark:border-surface-700">
                    <div class="flex items-center justify-between px-5 py-4 border-b border-surface-200 dark:border-surface-700">
                        <div>
                            <h3 class="font-semibold text-lg text-surface-900 dark:text-white">File d'attente</h3>
                            <p class="text-xs text-surface-500">{{ medecinQueue.length }} patients en attente</p>
                        </div>
                                        ...
                                        <div class="flex items-center justify-between mb-1">
                                            ...
                                            <span
                                                :class="[
                                                    ...
                                                    Number(consultation.state) === 1 ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-400'
                                                ]"
                                            >
