      ...
      <div v-else>
        <TransitionGroup name="notif-list" tag="div" class="space-y-2.5">
          <div
            ...
            :class="[
              notif.status === 'non_vu'
                ? 'border-amber-200/70 bg-amber-50/40 dark:border-amber-800/40 dark:bg-amber-950/10'
                : 'border-surface-100 dark:border-surface-700/50 bg-surface-50/40 dark:bg-surface-700/20'
            ]"
          >
            <!-- Icone -->
            <div class="relative mt-0.5 shrink-0">
              <div
                ...
                :class="notif.status === 'non_vu' ? 'bg-amber-100 dark:bg-amber-900/40' : 'bg-surface-200 dark:bg-surface-700'"
              >
            ...
            <div class="shrink-0 flex flex-col items-end gap-1.5">
              <span class="text-[10px] px-2 py-0.5 rounded-full font-medium" :class="notif.status === 'non_vu' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400' : 'bg-surface-100 text-surface-400 dark:bg-surface-700 dark:text-surface-500'">
                {{ notif.status === 'non_vu' ? 'Non lue' : 'Lue' }}
              </span>
              <button v-if="notif.status === 'non_vu'" class="rounded-lg px-2 py-1 text-xs text-emerald-600 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-900/20 transition-colors flex items-center gap-1" @click="emit('mark-read', [notif.id])">
