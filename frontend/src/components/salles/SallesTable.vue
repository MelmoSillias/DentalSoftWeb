      ...
      <template #body="{ data }">
        ...
        <Column header="Actions" style="min-width: 120px">
            <template #body="{ data }">
                <div class="flex items-center gap-2">
                    ...
                    <Button icon="pi pi-pen-to-square" severity="info" text rounded v-tooltip.top="'Modifier'" class="hover:bg-blue-50 dark:hover:bg-blue-900/20" @click="$emit('edit', data)" />
                    <Button icon="pi pi-trash" severity="danger" text rounded v-tooltip.top="'Supprimer'" class="hover:bg-red-50 dark:hover:bg-red-900/20" @click="$emit('delete', data)" />
                </div>
            </template>
        </Column>
        ...
        <template #empty>
            <div class="text-center py-12">
                ...
                <h4 class="text-lg font-semibold text-surface-700 dark:text-surface-300 mb-2">Aucune salle trouvée</h4>
                <p class="text-surface-600 dark:text-surface-400 mb-6 max-w-md mx-auto">Commencez par ajouter votre première salle pour gérer vos espaces de consultation.</p>
                <Button label="Ajouter une salle" icon="pi pi-plus" @click="emitAdd" class="bg-gradient-to-r from-primary-500 to-primary-600 border-0 rounded-xl px-5 py-2.5" />
            </div>
        </template>
