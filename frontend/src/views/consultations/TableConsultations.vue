...
import { cancelConsultation, fetchConsultationDetails, fetchConsultationInvoice, fetchConsultationsByDate, updateConsultationInvoice } from '@/services/consultations';
...
const hasOpenDialogs = computed(() => showCreateDialog.value || quickDialogVisible.value || detailsDialogVisible.value || factureDialogVisible.value);
            ...
            <div class="bg-surface-0 dark:bg-surface-800/80 rounded-2xl shadow-xl overflow-hidden border border-surface-200/50 dark:border-surface-700/50 backdrop-blur-sm mb-6 md:mb-8">
                ...
                <DataTable
                    ...
                    :pt="{
                        ...
                        row: ({ data }) => ({
                            class: ['hover:bg-surface-50/50 dark:hover:bg-surface-700/30 transition-colors', data.statut === 'URGENT' && 'bg-red-50/30 dark:bg-red-900/10', data.statut === 'EN_COURS' && 'bg-blue-50/30 dark:bg-blue-900/10']
                        }),
                        paginator: {
                ...
                >
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <Tag v-if="data.hasInsurance" value="Assuré" severity="success" icon="pi pi-shield" class="text-xs" />
                                    </div>
                    ...
                    <Column header="Statut">
                        ...
                        <template #body="{ data }">
                            <div class="flex flex-col gap-1" :data-tour="data.id === 9202 ? 'consultations-table.status' : null">
                                <Tag :value="stateLabel(data).label" :severity="stateLabel(data).severity" class="px-3 py-1.5 rounded-full font-medium shadow-sm" />
                    ...
                    <Column header="Actions" :style="{ minWidth: '220px' }">
                        ...
                        <template #body="{ data }">
                            <div class="flex items-center gap-2" :data-tour="data.id === 9201 ? 'consultations-table.actions' : null">
                                <Button icon="pi pi-eye" severity="info" text rounded v-tooltip.top="'Voir détails'" class="hover:bg-blue-50 dark:hover:bg-blue-900/20" :loading="detailsLoadingId === data.id" @click="openDetails(data)" />
                                <Button
                                ...
                                <Button v-if="canUseQuickActions && !isClosed(data)" icon="pi pi-bolt" label="Actions rapides" severity="contrast" size="small" outlined class="rounded-xl" @click="toggleQuickActions($event, data)" />
                    ...
                    <template #empty>
                        <div class="text-center py-16">
                            ...
                            <h4 class="text-xl font-semibold text-surface-700 dark:text-surface-300 mb-3">Aucune consultation trouvée</h4>
                            ...
                            <div class="flex gap-3 justify-center">
                                <Button v-if="!isMedecin" icon="pi pi-plus" label="Créer une consultation" class="bg-gradient-to-r from-primary-500 to-primary-600 border-0" @click="showCreateDialog = true" />
                                <Button v-if="filterGlobalValue" icon="pi pi-filter-slash" label="Réinitialiser les filtres" severity="secondary" outlined @click="resetFilters" />
                            </div>
                        </div>
                    </template>
