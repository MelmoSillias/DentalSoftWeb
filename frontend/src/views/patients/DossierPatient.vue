...
import { activatePatientsTourMock, deactivatePatientsTourMock, getPatientsTourMockPatientIdForScenario, resetPatientsTourMockData, resolvePatientsTourMockScenario } from '@/services/patientsTourMock';
...
const {
    ...
    getPatientId: () => (guidedTourDemoActive.value ? (selectedPatientId.value ?? props.patientId ?? null) : null)
});

const isValidPatientId = (id) => id != null && Number.isFinite(Number(id)) && Number(id) > 0;

const currentPatientId = computed(() => {
    const candidate = guidedTourDemoActive.value ? (selectedPatientId.value ?? props.patientId ?? patient.value?.id ?? null) : (props.patientId ?? null);
...
const loadPatientOptions = async (query = '') => {
    ...
    try {
        ...
        const response = isMedecin.value ? await patientStore.fetchPatientsByMedecin(token, params) : await patientStore.fetchPatients(token, params);
        ...
        const keepId = isValidPatientId(selectedPatientId.value) ? selectedPatientId.value : isValidPatientId(props.patientId) ? props.patientId : null;
...
onMounted(async () => {
    ...
    const dossierPromise = initialId != null ? loadAll(initialId, { asPageLoad: true }) : null;
