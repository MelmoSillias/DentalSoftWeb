    ...
    <div class="p-4 space-y-4">
        ...
        <div class="p-4 border rounded shadow-sm bg-white">
            ...
            <div class="mt-4 flex items-center gap-3">
                <div>
                    <Tag v-if="rootStatus === 'ok'" size="large" class="text-lg" icon="pi pi-check" severity="success" value="Dossier racine sélectionné"></Tag>
                    <Tag v-else-if="rootStatus === 'no'" size="large" class="text-lg" icon="pi pi-times" severity="danger" value="Aucun dossier racine"></Tag>
                    <Tag v-else size="large" class="text-lg" icon="pi pi-info" severity="info" value="Vérification..."></Tag>
                </div>

                <div class="ml-auto flex gap-2">
                    <Button @click="selectRoot" label="Sélectionner" class="inline-flex items-center px-3 py-1.5 bg-indigo-600 text-white text-sm rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none" />
                    <Button v-if="rootStatus === 'ok'" @click="clearRoot" label="Révoquer" class="inline-flex items-center px-3 py-1.5 bg-gray-100 text-gray-800 text-sm rounded-md border hover:bg-gray-200" />
                    <Button v-if="rootStatus !== 'checking'" @click="requestRootPermission" label="Demander la permission" class="inline-flex items-center px-3 py-1.5 bg-yellow-500 text-white text-sm rounded-md hover:bg-yellow-600" />
                </div>
            </div>

            <div class="mt-3 text-sm text-gray-600">Note: les options d'ouverture et de création de fichiers/dossiers sont désactivées dans cette vue.</div>
        </div>
        ...
        <div class="p-4 border rounded shadow-sm bg-white">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Button
                        @click="createAll"
                        :disabled="creatingAll || !missingRows.length"
                        :loading="creatingAll"
                        label="Créer tout"
            ...
            <div class="mt-4">
                ...
                <div v-else>
                    <div v-if="!missingRows.length" class="text-sm text-gray-500">Aucun dossier manquant trouvé. Lancez une recherche pour vérifier.</div>
                        ...
                        <table class="w-full text-sm table-auto">
                            ...
                            <tbody>
                                <tr v-for="(r, idx) in missingRows" :key="idx" class="border-t">
                                    ...
                                    <td class="px-2 py-2">
                                        <Button @click="createForRow(r)" icon="pi pi-plus" label="Créer" class="p-button-sm p-button-rounded" />
                                    </td>
                                </tr>
                            </tbody>
                        </table>
