...
class RdvService
{
    ...
    public function createRdv(array $data): array
    {
        ...
        return $this->patientService->createRdv($payload);
    }
    ...
    private function createConsultationFromRdv(Rdv $rdv, ?int $medecinId = null): void
    {
    ...
    public function updateStatus(int $rdvId, int $newStatus, array $payload = []): array
    {
        ...
        if ($newStatus === 1) {
            $this->createConsultationFromRdv($rdv, $payload['medecin'] ?? null);
        }
    ...
    public function handleAction(Rdv $rdv, string $action, array $payload): array
    {
        if ($action === 'validate') {
            $rdv->setStatut(1);
            $this->createConsultationFromRdv($rdv, $payload['medecin'] ?? null);
